...
- (NSArray *) menuTitles{
	return [NSArray arrayWithObjects: NSLocalizedStringFromTableInBundle(@"BibTeX Library", nil, [NSBundle bundleForClass: [self class]], @"Name of first menu title"), nil];
}
...
- (void) performExportOfRecords: (NSDictionary *)records{
	...
	// Ready to rumble, in this case we do it after we already determine nr of items to export
	// so we immediately show correct progress. If setup takes long, begin with this method so
	// that indeterminate progress bar can already start off. Once you have found out the nr
	// of items, call updatestatus to change the progressbar to determinate.
	...
	if(!url){
        ...
        [userInfo setObject: NSLocalizedStringFromTableInBundle(@"No url found to write BibTeX file.", nil, [NSBundle bundleForClass: [self class]], @"") forKey:NSLocalizedDescriptionKey];
        [userInfo setObject: NSLocalizedStringFromTableInBundle(@"Please select a suitable location for the exported file.", nil, [NSBundle bundleForClass: [self class]], @"") forKey:NSLocalizedRecoverySuggestionErrorKey];
	...
	while(paper = [papers nextObject]){
		...
		// citekey, see if we have one already, otherwise create according to the Bibdesk default %a1 : %Y %r2 (McCracken:2004yc)
		...
		if(!citeKey){
			...
			citeKey = [NSString stringWithFormat: @"%@:%d%c%c", (authorString ? authorString : @"Untitled"), (year ? [year intValue] : [[NSCalendarDate calendarDate]yearOfCommonEra]), firstchar, secondchar];
	...
	if(err) {
        ...
        [userInfo setObject: NSLocalizedStringFromTableInBundle(@"Error while writing BibTeX file for endnote file", nil, [NSBundle bundleForClass: [self class]], @"") forKey:NSLocalizedDescriptionKey];
...
- (void) cancelExport{
	...
	[self setStatusString: NSLocalizedStringFromTableInBundle(@"Cancelling export...", nil, [NSBundle bundleForClass: [self class]], @"Status message shown while cancelling export.")];
}
